...
public class UserExtractionTest extends TestBase {
    ...
    public void test010() { // This name is not available so i choose random one
        List<String> email = response.extract().path("findAll{it.name=='Mani Banerjee'}.email");
        System.out.println("------------------StartingTest---------------------------");
        System.out.println("the object where name is Mani Banerjee : " + email);
        System.out.println("------------------End of Test---------------------------");
    }

    @Test
    //11. Get gender of id = 5914189
    public void test011() { // This ID is not available so i choose the closest number
        List<?> id = response.extract().path("findAll{it.id=='5914149'}.gender");
        System.out.println("------------------StartingTest---------------------------");
        System.out.println(" Gender of id is : 5914149" +
                " " + id);
        System.out.println("------------------End of Test---------------------------");
    }
}